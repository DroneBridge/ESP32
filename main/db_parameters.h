...
#define DB_PARAM_MAV_CNT                                                      \
  17 // Number of MAVLink parameters returned by ESP32 in the PARAM message.
     // Needed by GCS.
...
{
  uint8_t db_name[DB_PARAM_NAME_MAXLEN]; // equals the NVS key, JSON object
                                         // name and web interface input ID
  enum db_param_type_t type;
  struct
  { // no string support for mavlink
    uint8_t param_name[DB_PARAM_MAX_MAV_PARAM_NAME_LEN];
    uint16_t
      param_index; // strings will be ignored for mavlink set to -1 for strings
...
bool db_param_is_valid_u16(uint16_t new_u16_value,
                           db_parameter_t *target_param);
bool db_param_is_valid_i32(int32_t new_i32_value,
                           db_parameter_t *target_param);
