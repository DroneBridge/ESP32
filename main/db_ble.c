 ...
 * @authors Witty-Wizard <agarwalshashank429@gmail.com> further modified by
 *Wolfgang Christl
    ...
    .characteristics =
      (struct ble_gatt_chr_def[]){
        ...
        },
        {
          ...
          .access_cb = ble_svc_gatt_handler, // No direct access
                                             // required, only notify
...
/******************************************************************************
 * Private Function
 *Definitionsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
 *******************************************************************************/
 ...
 * @return 0 on success for all clients. -1 on failure to allocate buffer or
 * ble_gatts_notify_custom return value
 */
int
db_ble_send_data(const uint8_t *data, uint16_t data_len)
{
  for(int i = 0; i < CONFIG_BT_NIMBLE_MAX_CONNECTIONS; i++) {
    ...
    if(conn_handle_subs[i]) {
      ...
      int rc = ble_gattc_write_flat(i,
                                    ble_spp_svc_gatt_notify_val_handle,
                                    data,     /* Data pointer */
...
/**
 * @brief Sends BLE data, automatically splitting it into chunks if it exceeds
 * the MTU limit.
 *
 * @param data Pointer to the data buffer to send.
 * @param data_len Length of the data in the buffer.
 * @return 0 on success, negative error code on failure during sending. Returns
 * 1 if data_len is 0.
 */
int
send_ble_data_chunked(const uint8_t *data, uint16_t data_len)
{
  ...
  if(max_payload_size == 0) {
    ESP_LOGE(TAG,
             "Cannot send data, effective MTU is zero (ble_mtu_size: %d)",
             ble_mtu_size);
  ...
  if(data_len <= max_payload_size) {
    ...
    ESP_LOGD(TAG,
             "Sending single packet, len: %d, max_payload: %d",
             data_len,
             max_payload_size);
    ...
    ESP_LOGD(TAG,
             "Splitting data (len: %d) into chunks (max_payload: %d)",
             data_len,
             max_payload_size);
    uint16_t offset = 0;
    while(offset < data_len) {
      ...
      ESP_LOGD(
        TAG, "Sending chunk, offset: %d, chunk_size: %d", offset, chunk_size);

      // Send the current chunk (pointer arithmetic to get the chunk start)
      ret = db_ble_send_data(data + offset, chunk_size);
      if(ret != 0) {
        ESP_LOGE(
          TAG, "Failed to send chunk at offset %d, error: %d", offset, ret);
...
db_ble_server_uart_task()
{
  DB_RADIO_IS_OFF = false; // we are about to send stuff - update this
                           // parameter to indicate to others
  ...
  while(true) {
    ...
    if(xQueueReceive(db_uart_read_queue_ble, &bleData, 0)) {
      ...
      // Instead, we could also change DB_PARAM_SERIAL_PACK_SIZE dynamically
      // based on the BLE MTU, but that can result in very small buffers for
      // the UART task to read which may cause performance issues. The serial
      // packets arrive here with the length of DB_PARAM_SERIAL_PACK_SIZE.
      // Fragment again if necessary.
    ...
    if(delay_timer_cnt == 5000) {
      ...
      for reference */
      db_ble_request_rssi(
        &db_esp_signal_quality
           .air_rssi); // use this timer to get the RSSI updated
...
gap_event_handler(struct ble_gap_event *event, void *arg)
{
  ...
  case BLE_GAP_EVENT_MTU:
    ESP_LOGI(TAG,
             ...
             event->mtu.value);
    ble_mtu_size =
      event->mtu.value; // ToDo: Support more than one MTU for all connections
    break;
...
start_advertising(void)
{
  ...
  rc =
    ble_gap_adv_start(own_addr_type, // Type of address that stack should use
...
inline static void
format_addr(char *addr_str, uint8_t addr[])
{
  sprintf(addr_str,
                     ...
                     struct ble_gatt_access_ctxt *ctxt, void *arg)
{
  ...
  case BLE_GATT_ACCESS_OP_WRITE_CHR:
    ...
    if(bleData.data == NULL) {
      ESP_LOGE(
        TAG, "Failed to allocate memory for BLE data of length %d", len);
    ...
    if(xQueueSend(db_uart_write_queue_ble, &bleData, portMAX_DELAY) !=
       pdPASS) {
...
gap_init()
{
  ...
  if(rc != 0) {
    ESP_LOGE(
      TAG, "failed to set device name to %s, error code: %d", DEVICE_NAME, rc);
    return rc;
  }
...
nimble_host_config_init()
{
  ...
  /** WittyWizard:
   * TO DO: Figure out what they do later, code works without them, but what
 ...
 * @param rssi Pointer to the RSSI variable to be filled. The value is in dBm
 * (I suspect)
 */
void
db_ble_request_rssi(int8_t *rssi)
{
...
db_ble_init()
{
  ...
  xTaskCreate(db_ble_server_uart_task, /**< Task Function */
              ...
              5,    /**< Task Priority */
              NULL  /**< Task reference, used to change behaviour of task from
                       another task */
  );
...
db_ble_queue_init()
{
  ...
  if(db_uart_read_queue_ble == NULL) {
    ESP_LOGI(TAG,
             "Failed to create another queue, you are definitely on your own, "
             "KABOOM! KABOOM!");
    return;
  }
}
