...
#define USE_ESPRESSIF_EXAMPLE_HEADER 0  // send fragmented data with an additional header
...
//  [Length 1][Data Type 1][Data 1]
static const uint8_t spp_adv_data[20] = {
        /* Flags */
        2, ESP_BLE_AD_TYPE_FLAG, (ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT),
        /* Complete List of 16-bit Service Class UUIDs */
        0x03, ESP_BLE_AD_TYPE_16SRV_CMPL, 0x32, 0xDB,
        /* Complete Local Name in advertising */
        0x0C,ESP_BLE_AD_TYPE_NAME_CMPL, 'D', 'r', 'o', 'n', 'e', 'B', 'r', 'i', 'd', 'g', 'e'
};
...
static const uint8_t spp_data_notity_char_prop = ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_NOTIFY | ESP_GATT_CHAR_PROP_BIT_INDICATE;
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            ESP_ERROR_CHECK(esp_ble_gap_config_adv_data_raw((uint8_t *) spp_adv_data, sizeof(spp_adv_data)));
...
_Noreturn static void db_ble_server_uart_task() {
    ...
    while (true) {
        ...
        if (xQueueReceive(db_uart_read_queue_ble, &bleData, 0)) {
            ...
            if(!enable_data_ntf){
                ESP_LOGE(TAG, "%s did not enable data Notify", __func__);
                continue;
            }
