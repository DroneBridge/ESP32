...
/***************************************************************************************************************************
 * Configures the onboard USB/JTAG interface for serial communication (instead of an UART). Board must support this
 * interface.
 *
 ...
 */
static void db_ltm_check_arm_state_set_wifi(const msp_ltm_port_t *db_msp_ltm_port);
...
static esp_err_t open_uart_serial_socket() {
  ...
  if (DB_UART_PIN_TX > SOC_GPIO_IN_RANGE_MAX || DB_UART_PIN_RX > SOC_GPIO_IN_RANGE_MAX ||
      DB_UART_PIN_CTS > SOC_GPIO_IN_RANGE_MAX || DB_UART_PIN_RTS > SOC_GPIO_IN_RANGE_MAX) {
  ...
  ESP_ERROR_CHECK(uart_set_pin(UART_NUM, DB_UART_PIN_TX, DB_UART_PIN_RX, flow_control ? DB_UART_PIN_RTS : UART_PIN_NO_CHANGE,
...
#ifdef CONFIG_DB_SERIAL_OPTION_JTAG
  // open JTAG based serial socket for comms with FC or GCS via FTDI - special feature of official DB for ESP32 boards. Uses
  // the onboard USB for serial I/O with GCS. this is basically the GND-Station mode for the ESP32
      ...
      if (parse_msp_ltm_byte(db_msp_ltm_port, serial_byte)) {
        ...
        if (db_msp_ltm_port->parse_state == MSP_PACKET_RECEIVED) {
          ...
          *serial_read_bytes = 0;
          ...
          if (ltm_frames_in_buffer == DB_LTM_FRAME_NUM_BUFFER && (DB_LTM_FRAME_NUM_BUFFER <= MAX_LTM_FRAMES_IN_BUFFER)) {
    ...
    if (fmav_parse_and_check_to_frame_buf(&result, mav_parser_rx_buf, &fmav_status_serial, uart_read_buf[i])) {
      ...
      if (result.res == FASTMAVLINK_PARSE_RESULT_OK) {
        if (fmav_msg_is_for_me(db_get_mav_sys_id(), db_get_mav_comp_id(), &msg)) {
          ...
          handle_mavlink_message(&msg, tcp_clients, udp_conns, &fmav_status_serial, DB_MAVLINK_DATA_ORIGIN_SERIAL);
        } else {
...
void db_route_mavlink_response(uint8_t *buffer, uint16_t length, enum DB_MAVLINK_DATA_ORIGIN origin, int *tcp_clients,
  ...
  switch (origin) {
  case DB_MAVLINK_DATA_ORIGIN_SERIAL:
    write_to_serial(buffer, length);
    break;
  case DB_MAVLINK_DATA_ORIGIN_RADIO:
