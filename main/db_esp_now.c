...
db_esp_now_clients_list_t
    *db_esp_now_clients_list; // Local list of known ESP-NOW peers with the last RSSI value and noise
                              // floor and sequence number
...
static db_esp_now_packet_t db_esp_now_packet_global = {
    ...
    .db_esp_now_packet_header.packet_type = DB_ESP_NOW_PACKET_TYPE_DATA}; // make static so it is gets instanced only once
 ...
 **************************************************************************************************************************/
static void generate_pkcs5_key(const char *password, unsigned char *key, size_t keylen);
 ...
 * @return 0 on success, -1 in case of unknown error of mbedtls_gcm_crypt_and_tag or return value of
 * mbedtls_gcm_crypt_and_tag payload is encrypted and part of the db_esp_now_packet
 **************************************************************************************************************************/
static int db_encrypt_payload(db_esp_now_packet_t *db_esp_now_packet, uint8_t encrypt_payload_len);
 ...
 **************************************************************************************************************************/
static int db_decrypt_payload(db_esp_now_packet_t *db_esp_now_packet, uint8_t *decrypt_out_buffer,
                              uint8_t len_encrypted_data);

/***************************************************************************************************************************
 * Encrypts, authenticates and sends packet via ESP-NOW
 * @param db_esp_now_packet Packet to send
 * @param payload_length Value of payload_length_decrypted of db_esp_now_packet - saves us from creating a var inside the
 * function
 * @return false if no packet was sent, true if packet was sent (actual sending will be confirmed by the send-callback)
 **************************************************************************************************************************/
static bool db_espnow_encrypt_auth_send(db_esp_now_packet_t *db_esp_now_packet, const uint8_t payload_length);
 ...
 * @return false if no packet was sent, true if packet was sent (actual sending will be confirmed by the send-callback)
 **************************************************************************************************************************/
static bool db_read_uart_queue_and_send();
 ...
 **************************************************************************************************************************/
static int16_t update_peer_list(db_esp_now_clients_list_t *esp_now_clients, uint8_t broadcast_peer_mac[6]);

/***************************************************************************************************************************
 * @brief Processes received ESP-NOW data.
 *
 * This function handles received ESP-NOW packets. The ESP firmware ensures that only valid
 * packets are forwarded. The processing steps include:
 *
 * 1. Decrypt and authenticate the packet.
 * 2. Check if the client is known based on its MAC address.
 *    - If on the ground station (GND), update the RSSI value.
 *    - On the air side, only one GND peer is expected.
 * 3. Verify the sequence number and, if on GND, update the lost packet count.
 * 4. Write the payload to the UART queue for processing by the `control_espnow` task.
 *
 * @param data Pointer to the received raw data via ESP-NOW.
 * @param data_len Length of the received data in bytes.
 * @param src_addr MAC address of the source device.
 * @param rssi Received Signal Strength Indicator (RSSI) in dBm for this packet.
 **************************************************************************************************************************/
static void db_espnow_process_rcv_data(uint8_t *data, uint16_t data_len, uint8_t *src_addr, int8_t rssi);

/***************************************************************************************************************************
 * @brief ESP-NOW send callback function.
 *
 * This function is called from the WiFi task when an ESP-NOW transmission completes.
 *
 * @note Avoid performing lengthy operations in this callback. Instead, post necessary
 * data to a queue and handle it from the control module task.
 *
 * @warning Sending ESP-NOW data with too short an interval between transmissions
 * may cause out-of-order execution of this callback. It is recommended to send
 * the next ESP-NOW packet only after the previous send callback has returned.
 *
 * @param mac_addr MAC address of the recipient device.
 * @param status Transmission status, indicating success or failure.
 **************************************************************************************************************************/
 ...
 *
 * This function is called from the WiFi task when new ESP-NOW data is received.
 * It serves as the entry point for handling incoming packets.
 *
 * @note Avoid performing lengthy operations in this callback. Instead, post
 * the necessary data to a queue and process it in the control module task.
 **************************************************************************************************************************/
 ...
 **************************************************************************************************************************/
static db_esp_now_clients_list_t *db_espnow_broadcast_peers_list_create();

/***************************************************************************************************************************
 * @brief Destroy structure containing all ESP-NOW broadcast connection quality information
 * @param db_esp_now_clients Structure containing all ESP-NOW broadcast connection quality information
 **************************************************************************************************************************/
static void db_espnow_broadcast_peers_list_destroy(db_esp_now_clients_list_t *esp_now_clients);
...
/***************************************************************************************************************************
 * @brief Deinitializes the ESP-NOW module, including queues and AES context.
 *
 * This function performs a cleanup of all ESP-NOW-related resources, including:
 * - Releasing the AES-GCM encryption context.
 * - Deleting semaphore queues used for ESP-NOW communication.
 * - Destroying the broadcast peers list.
 * - Deinitializing the ESP-NOW module.
 *
 * @warning The function deletes queues that may still be in use by the control task.
 * Ensure that all tasks depending on these queues are stopped before calling this function.
 **************************************************************************************************************************/
static void deinit_espnow_all();

/***************************************************************************************************************************
 * @brief Init all relevant structures and Queues for ESP-NOW communication
 ...
 **************************************************************************************************************************/
static bool db_espnow_schedule_internal_telemetry_packet();
 ...
 **************************************************************************************************************************/
static _Noreturn void process_espnow_data();
/***************************************************************************************************************************
...
static void generate_pkcs5_key(const char *password, unsigned char *key, size_t keylen) {
  ...
  ret = mbedtls_pkcs5_pbkdf2_hmac_ext(MBEDTLS_MD_SHA256, (const unsigned char *)password, strlen(password), generic_salt,
                                      strlen((char *)generic_salt), 10000, keylen, key);
  switch (ret) {
  case 0:
    // success
    break;
...
static int db_encrypt_payload(db_esp_now_packet_t *db_esp_now_packet, uint8_t encrypt_payload_len) {
  ...
  int ret = mbedtls_gcm_crypt_and_tag(
      &aes, MBEDTLS_GCM_ENCRYPT, encrypt_payload_len, db_esp_now_packet->db_esp_now_packet_header.aes_iv,
      DB_ESPNOW_AES_IV_LEN, (uint8_t *)&db_esp_now_packet->db_esp_now_packet_header, db_esp_now_packet_header_len,
      (uint8_t *)&db_esp_now_packet->db_esp_now_packet_protected_data,
      (uint8_t *)&db_esp_now_packet->db_esp_now_packet_protected_data, DB_ESPNOW_AES_TAG_LEN, db_esp_now_packet->tag);
...
static int db_decrypt_payload(db_esp_now_packet_t *db_esp_now_packet, uint8_t *decrypt_out_buffer,
  ...
  int ret_decrypt = mbedtls_gcm_auth_decrypt(
      &aes, len_encrypted_data, db_esp_now_packet->db_esp_now_packet_header.aes_iv, DB_ESPNOW_AES_IV_LEN,
      (uint8_t *)&db_esp_now_packet->db_esp_now_packet_header, db_esp_now_packet_header_len, db_esp_now_packet->tag,
      DB_ESPNOW_AES_TAG_LEN, (uint8_t *)&db_esp_now_packet->db_esp_now_packet_protected_data, decrypt_out_buffer);
  switch (ret_decrypt) {
  case 0:
    // Successfully decrypted and authenticated
    break;
  case MBEDTLS_ERR_GCM_AUTH_FAILED:
    ESP_LOGW(TAG, "MBEDTLS_ERR_GCM_AUTH_FAILED: Authenticated decryption failed.");
    break;
  case MBEDTLS_ERR_GCM_BAD_INPUT:
    ESP_LOGW(TAG, "MBEDTLS_ERR_GCM_BAD_INPUT: Bad input parameters to function.");
    break;
  default:
...
static bool db_espnow_encrypt_auth_send(db_esp_now_packet_t *db_esp_now_packet, const uint8_t payload_length) {
  ...
  ret = db_encrypt_payload(db_esp_now_packet,
                           (*db_esp_now_packet).db_esp_now_packet_protected_data.payload_length_decrypted + 1);
  if (ret == 0) {
    err = esp_now_send(BROADCAST_MAC, (const uint8_t *)db_esp_now_packet,
                       (db_esp_now_packet_header_len + DB_ESPNOW_AES_TAG_LEN + payload_length + 1));
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Error (%s) sending ESP-NOW data - size %i !", esp_err_to_name(err),
               (db_esp_now_packet_header_len + DB_ESPNOW_AES_TAG_LEN + payload_length + 1));
      return false;
    } else {
      return true;
...
static bool db_read_uart_queue_and_send() {
  ...
  if (db_espnow_send_queue != NULL && xQueueReceive(db_espnow_send_queue, &evt, 0) == pdTRUE) {
    ...
    // ToDo: Potential to optimize: esp_now_send() does not need an instance of buffer after sending, however using evt.data
    // as pointer resulted in errors
    ...
    if (db_espnow_encrypt_auth_send(&db_esp_now_packet_global, evt.data_len)) {
      ...
      if (db_esp_now_packet_global.db_esp_now_packet_header.seq_num < UINT32_MAX) {
        db_esp_now_packet_global.db_esp_now_packet_header
            .seq_num++; // packet is static so just increase number once we sent it
      } else {
...
static void db_espnow_process_rcv_data(uint8_t *data, uint16_t data_len, uint8_t *src_addr, int8_t rssi) {
  ...
  if (db_decrypt_payload(db_esp_now_packet, db_decrypted_data, len_payload) == 0) {
    ...
    if (peer_index != -1) {
      ...
      if (db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index].last_seq_num <
...
static void db_espnow_send_callback(const uint8_t *mac_addr, esp_now_send_status_t status) {
  ...
  if (status == ESP_NOW_SEND_FAIL) {
    ESP_LOGE(TAG, "Failed to send ESP-NOW packet - MAC: %02xh:%02xh:%02xh:%02xh:%02xh:%02xh", mac_addr[0], mac_addr[1],
...
static void db_espnow_receive_callback(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len) {
  ...
  if (data[0] == DB_ESPNOW_ORIGIN_GND && DB_RADIO_MODE == DB_WIFI_MODE_ESPNOW_GND) {
    ...
    defined(CONFIG_IDF_TARGET_ESP32C2) || defined(CONFIG_IDF_TARGET_ESP32S2)
  ...
  if (db_espnow_send_recv_callback_queue != NULL &&
      xQueueSend(db_espnow_send_recv_callback_queue, &evt, ESPNOW_MAXDELAY) != pdTRUE) {
...
/***************************************************************************************************************************
 * @brief Initializes and starts the ESP-NOW module.
 *
 * This function creates a task to handle ESP-NOW communication.
 *
 **************************************************************************************************************************/
void db_start_espnow_module() {
  xTaskCreate(&process_espnow_data, /**< Task function for ESP_NOW communication */
              "db_espnow",          /**< Task name (for debugging) */
