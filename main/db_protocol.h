...
#define DB_RAW_OFFSET                                                         \
  14 // when adhering the 802.11 header the payload is offset to not be
     // overwritten by SQN
#define MAX_DB_DATA_LENGTH                                                    \
  (RADIOTAP_LENGTH + DB_RAW_V2_HEADER_LENGTH +                                \
   DATA_UNI_LENGTH) // max length of a db raw packet
...
#define NUM_CHANNELS                                                          \
  14 // max number of channels sent over DroneBridge control module (ground)
#define DB_RC_NUM_CHANNELS                                                    \
  12 // number of channels supported by DroneBridge RC protocol

#define DB_PORT_CONTROLLER 0x01
#define DB_PORT_TELEMETRY                                                     \
  0x02 // deprecated. Use proxy port for bidirectional telemetry
...
#define APP_PORT_STATUS                                                       \
  1602 // for all kinds of status protocol messages. Same port on ground
       // station and app
#define APP_PORT_COMM 1603
#define APP_PORT_TELEMETRY                                                    \
  1604 // accepts MAVLink and LTM telemetry messages. Non MAVLink telemetry
       // messages get rerouted internally to APP_PORT_PROXY
#define PORT_TCP_SYSLOG_SERVER 1605
#define APP_PORT_PROXY 5760 // use this port for all MAVLink messages (TCP)
#define APP_PORT_PROXY_UDP                                                    \
  14550                     // use this port for all MAVLink messages (UDP)
#define APP_PORT_VIDEO 5000 // app accepts raw H.264 streams
#define APP_PORT_VIDEO_FEC                                                    \
  5001 // app accepts raw DroneBridge video stream data, performs FEC on
       // Android device
...
#define DB_AP_CLIENT_IP                                                       \
  "192.168.2.1" // default IP address of GCS connected via WiFi AP
