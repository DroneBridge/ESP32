 ...
 * Variable
 **************************************************************************************************************************/
int8_t num_connected_tcp_clients = 0; ///< Global Variable defined in global.h, initialized here
 ...
 **************************************************************************************************************************/
static void db_send_to_all_udp_clients(udp_conn_list_t *n_udp_conn_list, const uint8_t *data, uint data_length);
 ...
 **************************************************************************************************************************/
static void read_process_serial_link(int *tcp_clients, uint *transparent_buff_pos, uint *msp_ltm_buff_pos,
                                     uint8_t *msp_message_buffer, uint8_t *serial_buffer, msp_ltm_port_t *db_msp_ltm_port);

/***************************************************************************************************************************
 * @brief Thread that manages all incoming and outgoing ESP-NOW and serial (UART) connections. Called only when ESP-NOW mode
 *is selected
 **************************************************************************************************************************/
 ...
 * @param sta_list
 **************************************************************************************************************************/
static void db_send_internal_telemetry_to_stations(int tel_sock, wifi_sta_list_t *sta_list, udp_conn_list_t *udp_conns);
 ...
 **************************************************************************************************************************/
static void handle_internal_telemetry(int tel_sock, uint8_t *udp_buffer, socklen_t *sock_len,
                                      struct sockaddr_in *udp_client);

/***************************************************************************************************************************
 * @brief Thread that manages all incoming and outgoing TCP, UDP and serial (UART) connections. Called when Wi-Fi modes are
 *selected
 **************************************************************************************************************************/
static _Noreturn void control_module_udp_tcp();

/***************************************************************************************************************************
 * @brief Thread that manages all incoming and outgoing BLE and Serial Connections
 **************************************************************************************************************************/
...
static int db_open_int_telemetry_udp_socket() {
  ...
  if (DB_RADIO_MODE == DB_WIFI_MODE_STA) {
    ...
    if (!IP_MULTICAST(ntohl(imreq.imr_multiaddr.s_addr))) {
      ESP_LOGW(TAG, "Configured IPV4 multicast address '%s' is not a valid multicast address. This will probably not work.",
               MULTICAST_IPV4_ADDR);
    }
...
static void handle_internal_telemetry(int tel_sock, uint8_t *udp_buffer, socklen_t *sock_len,
                                      struct sockaddr_in *udp_client) {
  if (tel_sock > 0) {
    ssize_t recv_length = recvfrom(tel_sock, udp_buffer, UDP_BUF_SIZE, 0, (struct sockaddr *)udp_client, sock_len);
    if (recv_length > 0) {
...
static _Noreturn void control_module_udp_tcp() {
  ...
  while (1) {
    ...
    for (int i = 0; i < CONFIG_LWIP_MAX_ACTIVE_TCP; i++) { // handle TCP clients
      if (tcp_clients[i] > 0) {
        ...
        default:
          if (DB_SERIAL_PROTOCOL == DB_SERIAL_PROTOCOL_MAVLINK) {
            // Parse, so we can listen in and react to certain messages - function will send parsed messages to serial link.
            // We can not write to serial first since we might inject packets and do not know when to do so to not "destroy"
            // an existing packet
    ...
    if (DB_RADIO_MODE == DB_WIFI_MODE_STA && udp_broadcast_skybrush_socket != -1) {
      // This is special support for Skybrush. Skybrush sends some UDP broadcast msgs to 14555 in addition to regular msgs to
      // 14550 We only read and directly forward here. No parsing and no adding to known UDP clients
    ...
    read_process_serial_link(tcp_clients, &transparent_buff_pos, &msp_ltm_buff_pos, msp_message_buffer, serial_buffer,
                             &db_msp_ltm_port);
  ...
  for (int i = 0; i < n_udp_conn_list->size; i++) { // Loop through the array
    if (memcmp(n_udp_conn_list->db_udp_clients[i].mac, new_db_udp_client.mac,
      ...
      for (int j = i; j < n_udp_conn_list->size - 1; j++) { // Loop from the current index to the end of the array
        n_udp_conn_list->db_udp_clients[j] = n_udp_conn_list->db_udp_clients[j + 1]; // Shift the array elements to the left
      }
...
bool add_to_known_udp_clients(udp_conn_list_t *n_udp_conn_list, struct db_udp_client_t new_db_udp_client, bool save_to_nvm) {
  ...
  for (int i = 0; i < n_udp_conn_list->size; i++) {
    if ((n_udp_conn_list->db_udp_clients[i].udp_client.sin_port == new_db_udp_client.udp_client.sin_port) &&
        (n_udp_conn_list->db_udp_clients[i].udp_client.sin_addr.s_addr == new_db_udp_client.udp_client.sin_addr.s_addr)) {
