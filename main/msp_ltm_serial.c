 ...
 */
bool
parse_msp_ltm_byte(msp_ltm_port_t *msp_ltm_port, uint8_t new_byte)
{
  ...
  case LTM_TYPE_IDENT:
    ...
    msp_ltm_port->ltm_frame_buffer[2 + msp_ltm_port->ltm_payload_cnt] =
      new_byte;
    ...
    case LTM_TYPE_N:
    case LTM_TYPE_X:
      if(msp_ltm_port->ltm_payload_cnt == LTM_TYPE_A_PAYLOAD_SIZE)
        msp_ltm_port->parse_state = LTM_CRC;
      break;
    case LTM_TYPE_G:
    case LTM_TYPE_O:
      if(msp_ltm_port->ltm_payload_cnt == LTM_TYPE_G_PAYLOAD_SIZE)
        msp_ltm_port->parse_state = LTM_CRC;
      break;
    case LTM_TYPE_S:
      if(msp_ltm_port->ltm_payload_cnt == LTM_TYPE_S_PAYLOAD_SIZE)
        msp_ltm_port->parse_state = LTM_CRC;
      break;
    default:
      msp_ltm_port->parse_state = IDLE;
  ...
  case LTM_CRC:
    msp_ltm_port->ltm_frame_buffer[3 + msp_ltm_port->ltm_payload_cnt] =
      new_byte;
    ...
    if(msp_ltm_port->offset == sizeof(mspHeaderV1_t)) {
      ...
      if(hdr->cmd == MSP_V2_FRAME_ID) {
        ...
        msp_ltm_port->parse_state =
          msp_ltm_port->dataSize > 0 ? MSP_PAYLOAD_V1 : MSP_CHECKSUM_V1;
      }
    }
    break;
  ...
  case MSP_HEADER_V2_OVER_V1:
    msp_ltm_port->inBuf[msp_ltm_port->offset++] = new_byte;
    msp_ltm_port->checksum1 ^= new_byte;
    msp_ltm_port->checksum2 =
      crc8_dvb_s2_table(msp_ltm_port->checksum2, new_byte);
    if(msp_ltm_port->offset ==
       (sizeof(mspHeaderV2_t) + sizeof(mspHeaderV1_t))) {
      mspHeaderV2_t *hdrv2 =
        (mspHeaderV2_t *)&msp_ltm_port->inBuf[sizeof(mspHeaderV1_t)];
      ...
      if(hdrv2->size > MSP_PORT_INBUF_SIZE) {
        ...
        msp_ltm_port->offset = 0;
        msp_ltm_port->parse_state = msp_ltm_port->dataSize > 0
