 ...
 **************************************************************************************************************************/
static void db_answer_mavlink_cmd_request_message(uint16_t requested_msg_id, uint8_t *buff,
                                                  enum DB_MAVLINK_DATA_ORIGIN origin, int *tcp_clients,
                                                  udp_conn_list_t *udp_conns, fmav_message_t *the_msg,
...
static void db_answer_mavlink_cmd_request_message(uint16_t requested_msg_id, uint8_t *buff,
  ...
  case FASTMAVLINK_MSG_ID_AUTOPILOT_VERSION: {
    ...
    uint16_t len = fmav_msg_command_ack_encode_to_frame_buf(buff, db_get_mav_sys_id(), db_get_mav_comp_id(), &a, status);
    db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);

    fmav_autopilot_version_t autopilot_version = {.board_version = 0,
    ...
    len = fmav_msg_autopilot_version_encode_to_frame_buf(buff, db_get_mav_sys_id(), db_get_mav_comp_id(), &autopilot_version,
                                                         status);
    db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);
  } break;
  default: {
...
uint8_t db_get_mav_comp_id() { return MAV_COMP_ID_TELEMETRY_RADIO; }

uint8_t db_get_mav_sys_id() { return DB_MAV_SYS_ID; }

uint16_t db_create_heartbeat(uint8_t *buff, fmav_status_t *fmav_status) {
  return fmav_msg_heartbeat_pack_to_frame_buf(
      buff,                              /**< Output buffer for the packed MAVLink message */
      db_get_mav_sys_id(),               /**< MAVLink system ID of the sender */
      db_get_mav_comp_id(),              /**< MAVLink component ID of the sender */
      MAV_TYPE_ONBOARD_CONTROLLER,       /**< MAVLink type (Onboard Controller) */
      MAV_AUTOPILOT_INVALID,             /**< Autopilot type (Invalid, as it's an onboard controller) */
      MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, /**< MAVLink mode flag (Custom mode enabled) */
      0,                                 /**< Custom mode (set to 0) */
      MAV_STATE_ACTIVE,                  /**< System state (Active) */
      fmav_status                        /**< MAVLink status struct for message tracking */
  );
}
...
uint16_t db_get_mavmsg_param(uint8_t *buff, fmav_status_t *fmav_status, uint16_t param_index, float_int_union *value,
  ...
  fmav_param_value_t fmav_param_value = {
      .param_value = value->f, .param_type = type, .param_count = DB_MAV_PARAM_CNT, .param_index = param_index};
  if (strlen(param_id) >= sizeof(fmav_param_value.param_id)) { // max size of param_id is 16 bytes
    memcpy(fmav_param_value.param_id, param_id, 16);
  } else {
    strcpy(fmav_param_value.param_id, param_id);
  }
  return fmav_msg_param_value_encode_to_frame_buf(
      buff,                 /**< Output buffer where the encoded MAVLink message will be stored */
      db_get_mav_sys_id(),  /**< Retrieves the system ID of the MAVLink device */
      db_get_mav_comp_id(), /**< Retrieves the component ID of the MAVLink device */
      &fmav_param_value,    /**< Pointer to the populated fmav_param_value_t structure containing parameter details */
      fmav_status           /**< Pointer to the MAVLink status structure for tracking message sequence */
  );
}
  ...
  if (strncmp(param_set_payload->param_id, "SYS_ESP32_MODE", 16) == 0) {
    if (float_int.uint8 < DB_WIFI_MODE_ESPNOW_END) { // check E_DB_WIFI_MODE for allowed modes
      DB_RADIO_MODE_DESIGNATED =
          float_int.uint8; // do not directly change DB_WIFI_MODE since it is not safe and constantly processed by other
                           // tasks. Save settings and reboot will assign DB_RADIO_MODE_DESIGNATED to DB_WIFI_MODE
      success = true;
    } else {
    ...
    if (float_int.uint8 == DB_SERIAL_PROTOCOL_MAVLINK || float_int.uint8 == DB_SERIAL_PROTOCOL_TRANSPARENT ||
...
void db_process_mavlink_command(fmav_command_long_t *the_command, fmav_message_t *the_msg, fmav_status_t *status,
  ...
  case MAV_CMD_REQUEST_MESSAGE: {
    ...
    db_answer_mavlink_cmd_request_message(req_msg_id, buff, origin, tcp_clients, udp_conns, the_msg, status);
  } break;
  default: {
    fmav_command_ack_t b = {.command          = the_command->command,
                            ...
                            .target_system    = the_msg->sysid,
                            .target_component = the_msg->compid};
    uint16_t len = fmav_msg_command_ack_encode_to_frame_buf(buff, db_get_mav_sys_id(), db_get_mav_comp_id(), &b, status);
...
void handle_mavlink_message(fmav_message_t *new_msg, int *tcp_clients, udp_conn_list_t *udp_conns,
  ...
  case FASTMAVLINK_MSG_ID_HEARTBEAT:
    if (origin == DB_MAVLINK_DATA_ORIGIN_SERIAL) {
      // we only process heartbeats coming from the UART (local device) since we also use it as a trigger to send our
      // heartbeat
      ...
      if (payload.autopilot == MAV_AUTOPILOT_INVALID && payload.type == MAV_TYPE_GCS) {
        ...
        if (DB_RADIO_MODE == DB_WIFI_MODE_STA || (DB_RADIO_MODE == DB_WIFI_MODE_ESPNOW_AIR)) {
          fmav_radio_status_t payload_r = {.fixed    = 0,
                                           .txbuf    = 0,
          ...
          uint16_t len = fmav_msg_radio_status_encode_to_frame_buf(buff, db_get_mav_sys_id(), db_get_mav_comp_id(),
                                                                   &payload_r, fmav_status);
          db_send_to_all_clients(tcp_clients, udp_conns, buff, len);
        } else if (DB_RADIO_MODE == DB_WIFI_MODE_AP && wifi_sta_list.num > 0) {
          // We assume ESP32 is not used in DB_WIFI_MODE_AP on the ground but only on the drone side -> We are in WiFi AP
          // mode and connected to the drone Send each connected client a radio status packet. ToDo: Only the RSSI of the
          // first client is considered. Easier for UDP since we have a nice list with mac addresses to use for mapping.
          // Harder for TCP -> no MAC addresses available of connected clients
          fmav_radio_status_t payload_r = {.fixed    = 0,
                                           .noise    = 0,
                                           .remnoise = 0,
                                           .remrssi  = wifi_sta_list.sta[0].rssi,
                                           .rssi     = -127,
                                           .rxerrors = 0,
                                           .txbuf    = 0};
          uint16_t len = fmav_msg_radio_status_encode_to_frame_buf(buff, db_get_mav_sys_id(), db_get_mav_comp_id(),
  ...
  case FASTMAVLINK_MSG_ID_PARAM_SET: {
    ...
    if (db_write_mavlink_parameter(&parame_set_payload)) {
      ...
      if (type != 0) {
        uint16_t len = db_get_mavmsg_param(buff, fmav_status, 0, &float_int, type, parame_set_payload.param_id);
        db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);
